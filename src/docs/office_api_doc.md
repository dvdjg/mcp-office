...
### Use Case 1: Convert Word to Markdown with Comments and Table Formatting
**Task**: Convert a Word document (`CV.docx`) to Markdown (`CV.md`), appending comments and specifying table format.

**Office MCP API Call**:
```bash
curl -X POST "http://localhost:3000/word/markdown/export" \
  -H "Content-Type: application/json" \
  -d '{
    "filePath": "/docs/CV.docx",
    "output": "/docs/CV.md",
    "comments": "append",
...
**Explanation of Parameters for `word/markdown/export`**:
- `filePath` (string, required): Path to the Word document (.docx).
- `output` (string, required): Path for the output Markdown file (.md).
- `imageDir` (string, optional, default: `images`): Directory to save extracted images.
- `imagePrefix` (string, optional): Prefix for extracted image filenames.
- `tableFormat` (string, optional, enum: `markdown`, `html_tables`, `full_html`, default: `markdown`):
    - `markdown`: (Default) Attempts to convert tables to Markdown table syntax. Best effort.
    - `html_tables`: Converts tables to HTML tags. The rest of the document is Markdown. Uses `turndown` for HTML-to-Markdown of non-table content if the primary conversion path involves HTML.
    - `full_html`: Converts the entire document to HTML.
- `zipOutput` (boolean, optional, default: `false`): If true, creates a ZIP archive.
- `zipFileName` (string, optional): Name for the output ZIP file.
- `comments` (string, optional, enum: `ignore`, `append`, `inline`, default: `ignore`): How to handle comments.
- `useComInterop` (boolean, optional, default: `false`): `true` for basic COM Interop conversion (limited formatting). `false` for richer library-based conversion (e.g., Mammoth).
- `cleanMarkdownEscapes` (boolean, optional, default: `true`): Removes unnecessary backslashes from Markdown generated by library path.

**JavaScript API Code (Conceptual - Office MCP handles internal logic)**:
```javascript
// Office MCP internally uses libraries like Mammoth.js or custom logic
// based on `useComInterop` and `tableFormat` parameters.
// For `tableFormat: 'html_tables'`, if the initial conversion is to HTML,
// `turndown` might be used to convert non-table parts back to Markdown.

async function exportToMarkdownWithAdvancedOptions(filePath, output, options) {
  // 1. Read the Word document (e.g., using Mammoth.js if useComInterop is false)
  // 2. Process content based on `tableFormat`:
  //    - If 'markdown': Convert tables to Markdown syntax.
  //    - If 'html_tables': Convert tables to HTML, other content to Markdown (potentially via HTML then turndown).
  //    - If 'full_html': Convert entire document to HTML.
  // 3. Handle images: Extract to `imageDir` with `imagePrefix`.
  // 4. Handle comments based on `comments` option.
  // 5. Clean Markdown escapes if `cleanMarkdownEscapes` is true.
  // 6. Save the result to `output`.
  // 7. If `zipOutput` is true, create a ZIP archive.
  console.log(`Simulating export of ${filePath} to ${output} with options:`, options);
}
```

**VBA Code (Conceptual for COM Interop path - `useComInterop: true`)**:
```vba
Sub ExportToMarkdownWithOptions()
    ' This path is more basic and typically extracts text.
    ' Advanced table/image handling is usually done by external libraries in Office MCP.
    Dim doc As Document
    ' Set doc = Application.Documents.Open("C:\path\to\your\document.docx") ' Or ActiveDocument

    Dim markdownOutput As String
    markdownOutput = doc.Content.Text ' Basic text extraction

    ' Comment handling (simplified)
    If LCase(options.comments) = "append" Then
        Dim cmt As Comment
        For Each cmt In doc.Comments
            markdownOutput = markdownOutput & vbCrLf & "> " & cmt.Author & ": " & cmt.Range.Text
        Next cmt
    End If

    ' Office MCP would then save markdownOutput to the specified file.
    ' Table and image handling for COM path is rudimentary.
    ' For richer features, Office MCP uses libraries when useComInterop is false.
    Debug.Print "Markdown (basic COM extraction): " & markdownOutput
End Sub
```

**AI Notes**:
- **What’s Happening**: Office MCP uses libraries (like Mammoth.js when `useComInterop` is `false`) or COM Interop to convert Word to Markdown. The `tableFormat` parameter dictates how tables are handled. For `html_tables`, if the conversion process involves an intermediate HTML step, `turndown` helps convert non-table HTML parts back to Markdown. Images can be extracted, and the output can be zipped.
- **Troubleshooting**: Ensure `CV.docx` exists. Check the `tableFormat` option if tables are not appearing as expected. Verify `imageDir` if images are missing.
- **Why Use Office MCP**: Office MCP abstracts the complexity of different conversion paths, table strategies (including using `turndown` where appropriate), image extraction, and ZIP packaging.
