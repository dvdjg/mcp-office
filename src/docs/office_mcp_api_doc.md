...
**Description**: Converts Word to Markdown. This tool supports various table conversion strategies and comment handling. It can leverage COM Interop for basic text extraction or more advanced library-based conversion for richer Markdown, including table formatting and image handling.
...
- **extract**: Exports to Markdown.
  - **Input**:
    ...
    - `imageDir` (string, optional, default: `images`): Relative directory to save extracted images.
    - `imagePrefix` (string, optional): Optional prefix for extracted image filenames.
    - `tableFormat` (string, optional, enum: `markdown`, `html_tables`, `full_html`, default: `markdown`): Specifies how to handle tables:
        - `markdown`: (Default) Attempts to convert tables to Markdown table syntax. Fidelity may vary.
        - `html_tables`: Converts tables to HTML tags within the Markdown document. The rest of the document is converted to Markdown. This option utilizes the `turndown` library for HTML-to-Markdown conversion of non-table content if the primary conversion path involves HTML.
        - `full_html`: Converts the entire document content to HTML.
    - `zipOutput` (boolean, optional, default: `false`): If true, creates a ZIP archive containing the Markdown file and image directory.
    - `zipFileName` (string, optional): Optional name for the output ZIP file.
    - `comments` (string, optional, enum: `ignore`, `append`, `inline`, default: `ignore`): How to handle comments (`ignore`, `append` to end, `inline` where supported).
    - `useComInterop` (boolean, optional, default: `false`): `true` uses COM Interop (basic text, limited formatting). `false` uses platform-independent libraries (e.g., Mammoth) for richer conversion.
    - `cleanMarkdownEscapes` (boolean, optional, default: `true`): If true, removes unnecessary backslash escapes from Markdown generated by the library path.
  - **Output**: `{ success: true, output: string }`.
  - **Example**:
    ```bash
...
**AI Request Examples**:
- User: “Export CV.docx to Markdown, append comments, convert tables to HTML, and save images in 'cv_images'.”
  - AI Infers: `POST /word/markdown/export` with `filePath="/docs/CV.docx"`, `output="/docs/CV.md"`, `comments="append"`, `tableFormat="html_tables"`, `imageDir="cv_images"`.
- User: "Convert my report 'AnnualReport.docx' to full HTML output named 'AnnualReport.html'."
  - AI Infers: `POST /word/markdown/export` with `filePath="/docs/AnnualReport.docx"`, `output="/docs/AnnualReport.html"`, `tableFormat="full_html"`.

**Completions**: File paths, comment modes (`ignore`, `append`, `inline`), table formats (`markdown`, `html_tables`, `full_html`), image directory names.
